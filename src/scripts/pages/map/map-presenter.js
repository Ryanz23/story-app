...
export default class MapPresenter {
  ...
  async afterRender() {
    try {
      ...
      const mapTiler = L.tileLayer(
        'https://api.maptiler.com/maps/streets/{z}/{x}/{y}.png?key=Get_your_own_MapTiler_key',
        {
  ...
  async loadStories(page = 1) {
    ...
    this.view.updatePagination(pagination, this.showAllStories, () =>
    ...
    this.view.setupFilters(
      ...
      (filterType) => {
        ...
        if (filterType === 'all') {
          ...
          filteredStories = this.allStories.filter(
            (story) => story.name === userName
          );
        } else if (filterType === 'recent') {
          filteredStories = this.allStories.slice(0, 5);
        }
        this.view.showFilteredStories(
          filteredStories,
          this.markersByStory,
          this.map
        );
