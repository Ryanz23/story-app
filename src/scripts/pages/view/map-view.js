export default class MapView {
    ...
    return `
      <section class="container">
        <div class="map-page-container">
          ...
          <p class="map-description">Berikut adalah peta yang menunjukkan lokasi cerita dari semua pengguna StoryApp.</p>
  ...
  showEmpty() {
    ...
    if (storiesList) {
      storiesList.innerHTML =
        '<p class="empty-state">Tidak ada cerita dengan lokasi untuk ditampilkan.</p>';
    }
  }
  ...
  updatePagination(pagination, showAllStories, loadMoreCallback) {
    ...
    if (showAllStories) {
      paginationContainer.innerHTML =
        '<div class="page-info">Menampilkan semua cerita</div>';
      return;
    }

    const hasMore =
      typeof pagination.hasMore !== 'undefined'
        ? pagination.hasMore
  ...
  showFilteredStories(filteredStories, markersByStory, mapInstance) {
    ...
    if (filteredStories.length === 0) {
      storiesList.innerHTML =
        '<p class="empty-state">Tidak ada cerita yang cocok dengan filter ini.</p>';
      return;
    }
